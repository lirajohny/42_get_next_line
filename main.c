...
int main(int argc, char *argv[]) {
        ...
        int fd = open("arquivo_saida.txt", O_RDWR | O_CREAT | O_TRUNC, S_IRUSR |
S_IWUSR); if (fd == -1) { perror("Erro ao criar o arquivo"); return 1;
        }
        printf("fd is : %i <\n", fd);
        ssize_t bytes_escritos = write(fd, conteudo_do_arquivo,
strlen(conteudo_do_arquivo)); if (bytes_escritos == -1) { perror("Erro ao
escrever no arquivo"); close(fd); return 1;
        }
        lseek(fd, 0, SEEK_SET);
...
int main(void) {
  ...
  fd = open("new", O_RDONLY | O_CREAT, S_IRUSR | S_IWUSR);
  if (fd == -1) {
