...
/*   Updated: 2023/11/10 09:29:03 by jlira            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int call = 1;
static char	*check_remain(char *remain, int pos)
{
	...
	if (!buff_remain || len == 0)
		return (NULL);
	while(remain[pos + i] != '\0')
	{
		buff_remain[i] = remain[pos + i];
		i++;
	}
	buff_remain[i] = '\0';
...
static char	*read_remain(char **remain, int *bytes_read)
{
...
static char	*read_file(int fd, char **remain, int *i)
{
	...
	bytes_read = 0;
	if (call == 1)
		printf("\033[1;32m  before : \033[0m |\033[1;34m %s \033[0m|\n", *remain);
    if (!*remain)
    {
        *remain = ft_strdup("");
        if (!*remain)
	...
	while ((bytes_read = read(fd, buffer, BUFFER_SIZE)) > 0)
	{
		...
		content = ft_strjoin(content, ft_strdup(buffer));
		printf("\t\t => new->content (new) |\033[1;34m %s \033[0m|\n", content);
	}
...
char	*get_next_line(int fd)
{
	...
	if ((next_line = read_remain(&remain, &bytes_read)) != NULL)
		return (next_line);
	else
	{
